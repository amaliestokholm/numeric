...
print('Given the matrix A with dimensions {}x{}, where A in this run is\n{}'.format(*A.shape, A))
print('We try to solve the equationUsing Gram-Schmidt as earlier')
qr_gs_decomp(A=A, R=R)
qr_gs_solve(Q=A, R=R, b=b)
...
print('Are the two solutions equal? {}'.format(np.allclose(b, b_check, rtol))) 
qr_gv_inverse(A_gv, Ai)
AAi = np.dot(A_check, A_gv)
eye = np.identity(n)
print(AAi)
